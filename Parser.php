...
class Parser
{
    ...
    /**
     * Returns true if the given code point is a printable character and
     * not a emoji modifier or combine character.
     *
     * @param string $codePoint
     * @return bool
     */
    public static function codePointIsEmoji(string $codePoint): bool
    {
        if (\in_array($codePoint, self::$skinTones, true)) {
            return false;
        }
        if ($codePoint === self::$combiningCharacter) {
            return false;
        }
        if ($codePoint === self::$zeroWidthJoiner) {
            return false;
        }
    ...
    public function parse(string $string): EmojiCollection
    {
        $previousEncoding = \mb_internal_encoding();
        // Ensure the internal encoding is UTF-8
        \mb_internal_encoding('UTF-8');

        $emojiCollection = new EmojiCollection();
        foreach ($this->extractUnicodeSequences($string) as $group) {
            $emojiCollection->add(new Emoji($this->extractUnicodePoints($group)));
        }
    ...
    private function extractUnicodeSequences($string): array
    {
        return \preg_match_all('/' . self::IGNORE_REGEXP . '|' . self::UNICODE_EMOJI_REGEXP . '/u', $string, $matches) !== false ? $matches[0] : [];
    }

    private function extractUnicodePoints($string): array
    {
        $unicodePoints = [];
        $stringLength = \mb_strlen($string);

        for ($i = 0; $i < $stringLength; $i++) {
            $unicodePoints[] = \strtoupper(\dechex(\mb_ord(\mb_substr($string, $i, 1), 'utf-8')));
        }
